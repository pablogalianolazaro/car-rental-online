...
describe(URL, function () {
  ...
  it(`POST ${URL}/signup`, async function () {
    let USUARIO = {_email:"lumontanana@gmail.com",_password:"1234"};
    let response = await chai.request(URL).post(`/signin`).send(USUARIO);
    assert.equal(response.status, 200);
    assert.isTrue(response.ok);
    let usuario = response.body;
    assert.exists(usuario._email);
    assert.deepEqual(usuario._email, USUARIO._email);
    /**response = await chai.request(URL).get(`/signup/${reserva._id}`).send();
    assert.equal(response.status, 200);
    assert.isTrue(response.ok);
    let respuesta = response.body;
  ...
  it(`POST ${URL}/signup`, async function () {
    let USUARIO = {_email:"lumontanana@gmail.com",_password:"1234"};
    let response = await chai.request(URL).post(`/signup`).send(USUARIO);
    assert.equal(response.status, 200);
    assert.isTrue(response.ok);
    let usuario = response.body;
    assert.exists(usuario._email);
  ...
  it(`POST ${URL}/reservas/entregar?numero=`, async function(){
    let numero = 1;
    let response = await chai
      .request(URL)
      .delete(`/reservas?numero=${numero}`)
